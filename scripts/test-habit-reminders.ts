...
async function main() {
  ...
  if (enableUserId) {
    log.header(`Activation des reminders pour l'utilisateur ${enableUserId}`);
    ...
    if (!profile) {
      log.error(`Profil Discord non trouvé pour l'utilisateur ${enableUserId}`);
      await pool.end();
      process.exit(1);
    }
  ...
  const totalUsers = await db.select({ id: schema.user.id }).from(schema.user);
...
`);
  ...
  if (specificUserId || usersWithRemindersEnabled.length > 0) {
    ...
    for (const userData of usersToCheck) {
      ...
      const progress = Math.round((completedCount / userHabits.length) * 100);
      console.log(
  ...
  if (shouldSend && usersWithRemindersEnabled.length > 0) {
    ...
    for (const userData of usersWithRemindersEnabled) {
      ...
      const incompleteHabits = habitsWithStatus.filter((h) => !h.isCompleted);
      ...
      try {
        ...
        else if (progress >= 25) embedColor = 0xe6_7e_22; // Dark orange
