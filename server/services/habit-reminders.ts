...
export const HabitRemindersService = {
    ...
    try {
      ...
      for (const userData of usersWithReminders) {
        ...
        if (userData.lastReminderSent) {
          const lastReminderDate = getDateInTimezone(userTimezone);
        ...
        const userHabits = await HabitRemindersService.getUserHabitsWithStatus(
          userData.userId,
          todayDate
        );
  ...
  sendTestReminder: async (userId: string): Promise<boolean> => {
    try {
      ...
      if (userHabits.length === 0) {
        console.error(`[HabitReminders] User ${userId} has no active habits`);
        return false;
      }
