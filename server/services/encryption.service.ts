...
export class EncryptionService {
    ...
    static init(): void {
        ...
        try {
            EncryptionService.key = Buffer.from(env.ENCRYPTION_KEY, "hex");
            EncryptionService.iv = Buffer.from(env.ENCRYPTION_IV, "hex");

            // Validate key and IV lengths
            if (EncryptionService.key.length !== 32) {
                throw new Error(
                    "ENCRYPTION_KEY must be 32 bytes (64 hex characters)"
                );
            }
            if (EncryptionService.iv.length !== 16) {
                throw new Error(
