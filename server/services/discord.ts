...
const RANK_ROLE_MAPPING: Record<string, string> = {
  NEW: DISCORD_ROLE_IDS.NEW,
  BEGINNER: DISCORD_ROLE_IDS.BEGINNER,
  RISING: DISCORD_ROLE_IDS.RISING,
  CHAMPION: DISCORD_ROLE_IDS.CHAMPION,
  EXPERT: DISCORD_ROLE_IDS.EXPERT,
  LEGEND: DISCORD_ROLE_IDS.LEGEND,
  MASTER: DISCORD_ROLE_IDS.MASTER,
  GRANDMASTER: DISCORD_ROLE_IDS.GRANDMASTER,
  IMMORTAL: DISCORD_ROLE_IDS.IMMORTAL,
};
...
export const DiscordService = {
  ...
  updateUserRole: async (discordId: string, rank: string): Promise<void> => {
    ...
    // Filtrer les rôles existants pour enlever les anciens rôles de rank
    const nonRankRoles = existingRoles.filter(
      (existingRoleId) => !rankRoleIds.includes(existingRoleId)
    );

    // Ajouter le nouveau rôle de rank
    const newRoles = [...nonRankRoles, roleId];

    // Valider que tous les rôles sont des strings (IDs Discord)
    const validRoles = newRoles.filter(
      (r) => typeof r === "string" && r.length > 0
    );

    if (validRoles.length !== newRoles.length) {
      console.error(`Invalid roles detected:`, {
        newRoles,
        validRoles,
        roleId,
        rank,
    ...
    if (!response.ok) {
      ...
      console.error(
        ...
        errorText,
        `DiscordId: ${discordId}, Rank: ${rank}, RoleId: ${roleId}, Roles sent: ${JSON.stringify(validRoles)}`
      );
      throw new Error(
        `Failed to update Discord user role: ${response.status} ${errorText}`
      );
    }
  },
  ...
  autoSyncUserRank: async (discordId: string, rank: string): Promise<void> => {
    console.info(
    ...
    try {
      ...
      console.info("✅ [Discord AutoSync] Synchronisation webhook réussie");
    } catch (webhookError) {
      console.warn(
      ...
      console.info("✅ [Discord AutoSync] Synchronisation API directe réussie");
    }
      ...
      for (const user of connectedUsers) {
        ...
        try {
          ...
          await db
            ...
            .where(eq(discordProfile.userId, user.id));

          successCount++;
          console.info(`✅ [Discord BulkSync] Succès pour ${user.discordId}`);
        } catch (error) {
  ...
  sendGoalCompletion: async (discordId: string, goal: Goal): Promise<void> => {
