...
export class CTraderSocket {
	...
	constructor(isLive: boolean = true) {
        ...
        try {
            ...
            // @ts-ignore
            const types = Object.keys(json.nested?.OpenApi?.nested || {});
            console.log(`[CTraderSocket] Loaded V8 ${types.length} types. Has TrendbarsReq: ${types.includes("ProtoOAGetTrendbarsReq")}`);
        } catch (e) {
	...
	connect(): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			this.socket.on("open", () => {
				console.log(`[CTraderSocket] Connected to cTrader WebSocket at wss://${this.host}:${this.port}`);
			...
			this.socket.on("close", (code, reason) => {
				console.log(`[CTraderSocket] WebSocket Closed. Code: ${code}, Reason: ${reason}`);
	...
	private handleMessage(data: Buffer) {
		try {
            ...
            if (message.payloadType === 50) { 
                 ...
                 try {
                     const ProtoOAErrorRes = this.root.lookupType("OpenApi.ProtoOAErrorRes");
                     const errorPayload = ProtoOAErrorRes.decode(message.payload) as any;
                     console.error(`[CTraderSocket] Received Error Res for ${clientMsgId}: ${errorPayload.errorCode} - ${errorPayload.description}`);
                     
                     if (clientMsgId && this.resolvers.has(clientMsgId)) {
                        this.resolvers.get(clientMsgId)?.reject(new Error(`cTrader API Error: ${errorPayload.errorCode} - ${errorPayload.description}`));
                        this.resolvers.delete(clientMsgId);
                     }
                 } catch (err) {
                     console.error(`[CTraderSocket] Failed to decode error response`, err);
                     if (clientMsgId && this.resolvers.has(clientMsgId)) {
                        this.resolvers.get(clientMsgId)?.reject(new Error("cTrader API Error (Unknown)"));
                        this.resolvers.delete(clientMsgId);
                     }
                 }
                 return;
			...
			if (message.clientMsgId && this.resolvers.has(message.clientMsgId)) {
	...
	private decodePayload(payloadType: number, payload: Uint8Array) {
		...
		if (!typeName) return payload; // Return raw if unknown
		...
		return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                if(this.resolvers.has(clientMsgId)) {
                    this.resolvers.get(clientMsgId)?.reject(new Error("Timeout waiting for cTrader response"));
                    this.resolvers.delete(clientMsgId);
                }
            }, 30000); // 30s timeout
		});
