...
export class CTraderClient {
    private readonly baseUrl = "https://api.spotware.com/connect";
    ...
    async getAccountList(): Promise<CTraderAccount[]> {
        ...
        try {
            ...
            console.error(
                "[CTraderClient] Error fetching account list:",
                error
            );
            return [];
        } finally {
    ...
    async getOpenPositions(
        ...
        try {
            ...
            if (Number.isNaN(ctidTraderAccountId)) {
            ...
            const distinctSymbols = [
                ...new Set(positions.map((p: any) => p.tradeData.symbolId)),
            ];
            ...
            for (const symbolId of distinctSymbols) {
                try {
                    ...
                    if (trendbars.trendbar && trendbars.trendbar.length > 0) {
                        const lastBar = trendbars.trendbar.at(-1);
                    ...
                    console.warn(
                        `[CTraderClient] Failed to fetch price for symbol ${symbolId}`,
                        e
                    );
                }
            }
            ...
            return positions.map((p: any) => {
                ...
                return {
                    ...
                    openTimestamp: p.tradeData?.openTimestamp
                        ? new Date(
                              Number(p.tradeData.openTimestamp)
                          ).toISOString()
            ...
            console.error(
                "[CTraderClient] Error fetching open positions:",
                error
            );
            // Fail gracefully
            return [];
    ...
    async getDealHistory(
        ...
        try {
            ...
            return deals.map((d: any) => ({
                ...
                swap: d.closePositionDetail?.swap
                    ? Number(d.closePositionDetail.swap) / 100
                    : 0,
                profit: d.closePositionDetail?.grossProfit
                    ? Number(d.closePositionDetail.grossProfit) / 100
                    : 0,
                executionTime: d.executionTimestamp
                    ? new Date(Number(d.executionTimestamp)).toISOString()
                    : new Date().toISOString(),
                ...
                closePositionDetail: d.closePositionDetail
                    ? {
                          closedVolume: d.closePositionDetail.closedVolume
                              ? Number(d.closePositionDetail.closedVolume) / 100
                              : 0,
                          balance: d.closePositionDetail.balance
                              ? Number(d.closePositionDetail.balance) / 100
                              : 0,
                      }
                    : undefined,
            }));
            ...
            console.error(
                "[CTraderClient] Error fetching deal history:",
                error
            );
            // Fail gracefully
            return [];
 ...
 */
export function mapCTraderTradeSide(tradeSide: "BUY" | "SELL"): "BUY" | "SELL" {
    return tradeSide;
}
