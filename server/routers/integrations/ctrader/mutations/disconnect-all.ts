...
export const disconnectAllCTraderHandler = disconnectAllCTraderBase.handler(
	async ({ context }) => {
		...
		console.log(`[disconnectAllCTrader] Disconnecting all cTrader for user: ${userId}`);
		...
		console.log(`[disconnectAllCTrader] Found ${connections.length} broker connections`);

		const journalIds = connections.map(c => c.journalId).filter(Boolean) as string[];
		...
		if (journalIds.length > 0) {
			...
			console.log(`[disconnectAllCTrader] Deleted ${deletedJournals} journals`);
		}
		...
		console.log(`[disconnectAllCTrader] Deleted ${deletedAccounts.length} OAuth accounts`);
