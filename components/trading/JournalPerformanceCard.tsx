                ...
                <div className="text-2xl font-bold text-white">
                  {(() => {
                    ...
                    if (winningTrades > 0 && losingTrades > 0) {
                      // Calculer le profit factor correctement
                      const totalWins = stats.totalTrades > 0 ? (stats.winRate / 100) * stats.totalTrades : 0;
                      const totalLosses = stats.totalTrades - totalWins;
                      
                      // Estimation basée sur le PnL total et la répartition
                      const totalPnL = Number(stats.totalPnL);
                      const avgWin = totalWins > 0 ? (totalPnL * (totalWins / stats.totalTrades)) / totalWins : 0;
                      const avgLoss = totalLosses > 0 ? Math.abs(totalPnL * (totalLosses / stats.totalTrades)) / totalLosses : 0;
