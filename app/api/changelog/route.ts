...
function formatChangeText(title: string, _body: string | null): string {
    // Clean up the title
    let text = title.trim();
...
export async function GET() {
    try {
        ...
        const prChanges: Array<ChangelogEntry & { date: string }> =
            mergedPRs.map((pr) => ({
        ...
        const directCommits: Array<ChangelogEntry & { date: string }> =
            allCommits
                .filter((commit) => {
            ...
            .map(([date, changes]) => ({
                ...
                type: changes.some((c) => c.type === "feature")
                    ...
                    : changes.some((c) => c.type === "fix")
                      ? "patch"
                      : "minor",
                title: `Updates for ${new Date(date).toLocaleDateString(
                    "en-US",
                    {
                        year: "numeric",
                        month: "long",
