...
export async function GET() {
    try {
        console.info("Diagnostic du système de rappels via API");
        ...
        const eligibleGoals = allGoals.filter(
            (goal) =>
                goal.remindersEnabled && !goal.isCompleted && goal.isActive
        );

        const now = new Date();
        const overdueReminders = eligibleGoals.filter(
            (goal) =>
        ...
        const diagnosis = {
            ...
            goalsWithReminders: allGoals.filter((goal) => goal.remindersEnabled)
                .length,
            ...
            recommendations: {
                ...
                enableReminders:
                    allGoals.filter((goal) => goal.remindersEnabled).length ===
                    0,
                connectDiscord: discordUsers.length === 0,
                remindersUpToDate:
                    overdueReminders.length === 0 && eligibleGoals.length > 0,
            },
        };
