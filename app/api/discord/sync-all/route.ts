...
export async function POST() {
    try {
        console.info("🔄 [API] Synchronisation globale demandée");

        const result = await DiscordService.syncAllConnectedUsers();

        console.info("✅ [API] Synchronisation globale terminée", result);
        ...
        console.error(
            "❌ [API] Erreur lors de la synchronisation globale:",
            error
        );
