...
import { account, brokerConnections, oauthStates } from "@/server/db/schema";
...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = NextResponse.redirect(
            new URL(
                `/trading/journals/${journalId}?success=ctrader_connected`,
                baseUrl
            )
        );
...
async function exchangeCodeForToken(code: string): Promise<TokenResponse> {
    ...
    const response = await fetch(
        "https://connect.spotware.com/oauth/v2/token",
        {
            method: "POST",
            headers: {
