...
export function CTraderConnection() {
    ...
    const { mutateAsync: disconnectJournal, isPending: isDisconnectingOne } = useMutation(
        orpc.integrations.ctrader.disconnectAccount.mutationOptions({
            onSuccess: () => {
                queryClient.invalidateQueries(
                    orpc.integrations.ctrader.getConnections.queryOptions({ input: {} })
                );
                queryClient.invalidateQueries(
            ...
            onError: (error) => {
                console.error("[CTraderConnection] Disconnect error:", error);
    ...
    const { mutateAsync: disconnectAll, isPending: isDisconnectingAll } = useMutation(
        orpc.integrations.ctrader.disconnectAll.mutationOptions({
            onSuccess: () => {
                ...
                queryClient.invalidateQueries(
                    orpc.integrations.ctrader.getAccounts.queryOptions({ input: {} })
                );
                queryClient.invalidateQueries(
                    orpc.trading.getJournals.queryOptions({ input: {} })
                );
            ...
            onError: (error) => {
                console.error("[CTraderConnection] Disconnect all error:", error);
    ...
    const handleDisconnectJournal = async (journalId: string, journalName?: string) => {
        if (!confirm(`Delete "${journalName || journalId}"? This will remove the journal and all its trades.`)) {
            return;
        }
    ...
    const handleDisconnectAll = async () => {
        if (!confirm("Delete ALL cTrader journals? This will remove all journals linked to cTrader and their trades.")) {
            return;
        }
        await disconnectAll({});
    };
    ...
    return (
        <div className="space-y-0">
            ...
            <div className="flex space-x-3 border border-white/10 bg-white/5 p-4 text-sm">
                ...
                <div className="flex-1 space-y-1">
                    <p className="font-medium text-neutral-50">
                        {isLoading ? "Loading..." : isConnected ? "cTrader Connected" : "cTrader Not Connected"}
                    </p>
            ...
            {hasConnections && (
                <div className="border-b border-x border-white/10 bg-white/2 divide-y divide-white/5">
                    {connections.map((conn) => (
                        <div key={conn.id} className="flex items-center justify-between p-3 text-sm">
                            ...
                            <Button
                                variant="ghost"
                                size="xs"
                                onClick={() => handleDisconnectJournal(conn.journalId, conn.journalName)}
            ...
            {hasOAuth && !hasConnections && (
                <div className="border-b border-x border-white/10 bg-white/2 p-4">
                    <p className="text-xs text-neutral-400">
                        ...
                        Create a journal and connect it to cTrader to start syncing trades.
                    </p>
                </div>
            )}
        </div>
