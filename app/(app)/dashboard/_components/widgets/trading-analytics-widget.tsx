...
import { InfoIcon, TrendingUp, TrendingDown, Calendar, BarChart3 } from "lucide-react";
...
export function TradingAnalyticsWidget({ stats, trades }: TradingAnalyticsWidgetProps) {
    // Calculate day of week distribution
    const dayDistribution = (() => {
        const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
        const dayData = days.map(day => ({ day, trades: 0, pnl: 0, wins: 0 }));

        if (!trades || trades.length === 0) return dayData;
        ...
        for (const trade of trades) {
            ...
            if (pnl > 0) dayData[dayIndex].wins++;
        }
    ...
    const bestDay = tradingDays.length > 0
        ? tradingDays.reduce((best, day) => day.pnl > best.pnl ? day : best, tradingDays[0])
        : null;
    const worstDay = tradingDays.length > 0
        ? tradingDays.reduce((worst, day) => day.pnl < worst.pnl ? day : worst, tradingDays[0])
        : null;
    ...
    const directionStats = (() => {
        if (!trades || trades.length === 0) return { long: 0, short: 0, longPnL: 0, shortPnL: 0 };
                    ...
                    {hasTrades ? (
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-3 cursor-help">
                                            <div className="flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 mb-1">
                                                <TrendingUp className="w-3 h-3" />
                                                Best trade
                                            </div>
                                            <span className="text-lg font-bold text-emerald-500">
                                                +{Number(bestTrade?.profitLossPercentage || 0).toFixed(2)}%
                                            </span>
                                        </div>
                                    </TooltipTrigger>
                                    <TooltipContent className="text-xs">
                                        Your single most profitable trade in percentage terms
                                    </TooltipContent>
                                </Tooltip>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div className="bg-rose-500/5 border border-rose-500/20 rounded-lg p-3 cursor-help">
                                            <div className="flex items-center gap-1.5 text-xs text-rose-600 dark:text-rose-400 mb-1">
                                                <TrendingDown className="w-3 h-3" />
                                                Worst trade
                                            </div>
                                            <span className="text-lg font-bold text-rose-500">
                                                {Number(worstTrade?.profitLossPercentage || 0).toFixed(2)}%
                                            </span>
                                        </div>
                                    </TooltipTrigger>
                                    <TooltipContent className="text-xs">
                                        Your single biggest losing trade in percentage terms
                                    </TooltipContent>
                                </Tooltip>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-secondary/50 rounded-lg p-3">
                                    ...
                                    <span className={cn(
                                        "text-lg font-bold",
                                        maxDrawdown > 10 ? "text-rose-500" : maxDrawdown > 5 ? "text-amber-500" : "text-foreground"
                                    )}>
                                ...
                                {avgHoldingTime && (
                                    <div className="bg-secondary/50 rounded-lg p-3">
                                        ...
                                        <span className="text-lg font-bold">{avgHoldingTime}</span>
                                    </div>
                                )}
                                {!avgHoldingTime && (
                                    <div className="bg-secondary/50 rounded-lg p-3">
                                        ...
                                        <span className="text-lg font-bold">{stats.closedTrades}</span>
                                    </div>
                                )}
                            </div>

                            {/* Long vs Short */}
                            {(directionStats.long > 0 || directionStats.short > 0) && (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        <div className="flex-1 bg-blue-500/10 rounded-lg p-2 text-center">
                                            <span className="text-xs text-blue-500 font-medium block">Long</span>
                                            <span className="text-sm font-bold">{directionStats.long}</span>
                                            <span className={cn(
                                                "text-[10px] block",
                                                directionStats.longPnL >= 0 ? "text-emerald-500" : "text-rose-500"
                                            )}>
                                                {directionStats.longPnL >= 0 ? "+" : ""}{directionStats.longPnL.toFixed(1)}%
                                            </span>
                                        </div>
                                        <div className="flex-1 bg-purple-500/10 rounded-lg p-2 text-center">
                                            <span className="text-xs text-purple-500 font-medium block">Short</span>
                                            <span className="text-sm font-bold">{directionStats.short}</span>
                                            <span className={cn(
                                                "text-[10px] block",
                                                directionStats.shortPnL >= 0 ? "text-emerald-500" : "text-rose-500"
                                            )}>
                                                {directionStats.shortPnL >= 0 ? "+" : ""}{directionStats.shortPnL.toFixed(1)}%
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {bestDay && worstDay && bestDay.day !== worstDay.day && (
                                        <div className="flex gap-2 text-[10px]">
                                            <span className="text-emerald-500">Best: {bestDay.day}</span>
                                            <span className="text-rose-500">Worst: {worstDay.day}</span>
                                        </div>
                                    )}
                                </div>
                                        ...
                                        <Tooltip key={day.day}>
                                            <TooltipTrigger asChild>
                                                <div className="flex-1 flex flex-col items-center gap-1 cursor-help">
                                                    <div className="w-full h-12 bg-secondary rounded-sm relative overflow-hidden">
                                                        <div
                                                            className={cn(
                                                                "absolute bottom-0 w-full transition-all rounded-sm",
                                                                day.pnl >= 0 ? "bg-emerald-500/60" : "bg-rose-500/60"
                                                            )}
                                                            style={{ height: `${(day.trades / maxDayTrades) * 100}%` }}
                                                        />
                                                    </div>
                                            ...
                                            <TooltipContent className="text-xs">
                                                ...
                                                <div className={day.pnl >= 0 ? "text-emerald-500" : "text-rose-500"}>
                                                    {day.pnl >= 0 ? "+" : ""}{day.pnl.toFixed(2)}%
                                                </div>
                                                {day.trades > 0 && (
                                                    <div>Win rate: {((day.wins / day.trades) * 100).toFixed(0)}%</div>
                                                )}
                                            </TooltipContent>
                                        </Tooltip>
                                    ))}
